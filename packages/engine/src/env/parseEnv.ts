// packages/engine/src/env/parseEnv.ts
import { EnvParseOptions, ParsedConfig, ParseError, EnvProfile } from "../core/types";

const BASIC_DEFAULTS: Required<Omit<EnvParseOptions, "profile" | "expandFrom">> = {
...
const DOTENV_DEFAULTS: Required<Omit<EnvParseOptions, "profile" | "expandFrom">> = {
  allowExportPrefix: true,
  allowEmptyValues: true,
  allowDuplicateKeys: true,
  stripInlineComments: true,
...
function detectProfile(opts: EnvParseOptions): EnvProfile {
  return (opts.profile ?? "dotenv") as EnvProfile;
}
 ...
 */
function stripInlineCommentSmart(value: string): string {
  let inSingle = false;
  let inDouble = false;
...
function expandVars(value: string, dict: Record<string, string>): string {
...
export function parseEnv(input: string, opts: EnvParseOptions = {}): ParsedConfig {
  const profile = detectProfile(opts);
  const defaults = profile === "basic" ? BASIC_DEFAULTS : DOTENV_DEFAULTS;

  const o: Required<EnvParseOptions> = {
    profile,
    expandFrom: opts.expandFrom ?? {},
    ...defaults,
    ...opts,
  } as Required<EnvParseOptions>;
  ...
  const recordDup = (key: string, lineNo: number) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (o.allowExportPrefix) {
      ...
      if (m) working = working.slice(m[0].length).trim();
    }
    ...
    if (o.stripInlineComments) value = stripInlineCommentSmart(value);
  ...
  if (duplicates.length > 0) warnings.push(`Found ${duplicates.length} duplicate key(s). Last value wins.`);
  if (errors.length > 0) warnings.push(`Found ${errors.length} parse error(s). Some lines were ignored.`);
