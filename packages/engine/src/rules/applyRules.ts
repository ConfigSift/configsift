// packages/engine/src/rules/applyRules.ts
import { DiffResult, DiffWithRisk, RiskFinding } from "../core/types";
import { Rule } from "./defaultRules";

function msg(rule: Rule, key: string): string {
  return typeof rule.message === "function" ? rule.message(key) : rule.message;
}

type Ctx = "added" | "removed" | "changed";
 ...
 */
export function applyRiskRules(diff: DiffResult, rules: Rule[]): DiffWithRisk {
  ...
  const pushFinding = (key: string, rule: Rule, context: Ctx, note?: string) => {
  ...
  const matchesKey = (rule: Rule, key: string): boolean => {
    return rule.keyPattern ? rule.keyPattern.test(key) : true;
  };

  const matchesValue = (rule: Rule, value?: string): boolean => {
    if (!rule.valuePattern) return true; // no value constraint
    if (value === undefined) return false; // valuePattern exists but no value provided
    return rule.valuePattern.test(value);
  };

  const matches = (rule: Rule, key: string, value?: string): boolean => {
  ...
  for (const c of diff.changed) {
    for (const rule of rules) {
      ...
      if (!rule.valuePattern) {
        if (matchesKey(rule, c.key)) pushFinding(c.key, rule, "changed");
        continue;
      }
      ...
      if (toHit) pushFinding(c.key, rule, "changed", "matches new value");
      if (fromHit && !toHit) pushFinding(c.key, rule, "changed", "matches old value");
    }
  }

  if (findings.length >= MAX_FINDINGS) {
    warnings.push(
      `Risk findings capped at ${MAX_FINDINGS}. Consider filtering rules or narrowing inputs to reduce noise.`
    );
